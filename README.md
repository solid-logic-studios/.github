# Welcome to Solid Logic Studios

### Engineering-First Software Solutions

We are a developer collective and future consultancy based in Victoria, Australia. We specialize in building scalable, maintainable, and high-performance web applications with a strict focus on software craftsmanship.

---

### 🛠 Our Philosophy
We don't just write code that works; we write code that lasts. Our development culture is built on:
* **SOLID Principles:** Robust architecture that is easy to extend and maintain.
* **DRY (Don't Repeat Yourself):** Efficient, modular codebases.
* **Domain-Driven Design:** Aligning software structure with real-world business logic.

### 🚀 Active Projects
* **[SCRD](https://github.com/solid-logic-studios/scrd):** A modern squash scoring and tracking platform.
* **Markdown Tools:** Utilities for seamless content conversion and rendering.

### 💻 Tech Stack
* **Languages:** TypeScript, JavaScript (Node.js)
* **Backend:** Express.js, RESTful APIs
* **Frontend:** React, Modern Web Technologies
* **Infrastructure:** Cloud-Native Architecture, CI/CD pipelines

---

### 📫 Contact
Interested in our work or looking for collaboration?
* **Location:** Ballarat / Melbourne, VIC
* **Email:** [hello@solidlogicstudios.com.au](mailto:hello@solidlogicstudios.com.au) *(Coming Soon)*
* **Website:** [solidlogicstudios.com.au](https://solidlogicstudios.com.au) *(Under Construction)*
